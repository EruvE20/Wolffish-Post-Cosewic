...
knitr::opts_chunk$set(echo = F, message=F, warning=F, error = F, prompt = F)
```
...
#bathy <- getNOAA.bathy(-50, -80, 35, 48, resolution=1, antimeridian = F, keep=F)
```

```{r, fig.cap="Distribution of Atlantic wolffish catches in the 4VsW RV Survey, 1986-2010, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of immature (<53 cm) and mature (>53 cm) Atlantic wolffish in the 4VsW RV Survey, 1986-2010, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the 4X RV Survey, 2012-2020, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of immature (<53 cm) and mature (>53 cm) Atlantic wolffish in the 4X RV Survey, 2012-2020, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Spring RV Survey, 1979-1985, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Summer RV Survey, 1970-2021, summed by 15 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of immature (<53 cm) and mature (>53 cm) Atlantic wolffish in the Summer RV Survey, 1970-2021, summed by 15 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Fall RV Survey, 1978-1984, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Redfish RV Survey, 1982-1988, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Georges Bank RV Survey, 1987-2021, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of immature (<53 cm) and mature (>53 cm) Atlantic wolffish in the Georges Bank RV Survey, 1987-2021, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Stratified total biomass (A) and abundance (B) of immature and immature Atlantic wolffish in the 4VsW RV Survey, 1986-2010.", fig.align='center'}
strat <- spring4VsW %>% 
  group_by(YEAR, STRAT) %>% 
  summarize(numsets = length(SETNO))

catch <- spring4VsW %>% 
  filter(TOTWGT > 0) %>% 
  group_by(YEAR, STRAT) %>% 
  summarize(weight = mean(TOTWGT/DIST)*1.75)

abun <- spring4VsW_lengths %>% 
  mutate(maturity = ifelse(FLEN > 53, "mature", "immature")) %>% 
  group_by(YEAR, STRAT, maturity) %>% 
  summarize(abundance = mean((length(FLEN)/DIST)*1.75))

abun <- pivot_wider(abun,
                    names_from = maturity, 
...
strat <- strat %>% 
  ...
  left_join(area, by="STRAT") %>% 
  mutate(biomass = weight*AREA.GSSTRATUM,
         mature_abun = mature*AREA.GSSTRATUM,
         immature_abun = immature*AREA.GSSTRATUM) %>% 
  replace(is.na(.), 0)

total <- strat %>% 
  group_by(YEAR) %>% 
  summarise(biomass = sum(biomass),
            mature = sum(mature_abun),
            immature = sum(immature_abun)) %>% 
...
ggplot(total, aes(x=YEAR, y=abundance, col=maturity, shape=maturity))+
  geom_point()+
  geom_line()+
  theme_classic()+
  labs(x = "Year", y = "Mean number per tow")+
  ggtitle("B")
```
  
  
```{r, fig.cap="Stratified total biomass (A) and abundance (B) of immature and immature Atlantic wolffish in the 4X RV Survey, 2012-2020.", fig.align='center'}
strat <- spring4X %>% 
  group_by(YEAR, STRAT) %>% 
  summarize(numsets = length(SETNO))
...
```{r, fig.cap="Stratified total biomass (A) and abundance (B) of immature and immature Atlantic wolffish in the Summer RV Survey, 1970-2021.", fig.align='center'}
strat <- summer %>% 
  group_by(YEAR, STRAT) %>% 
  summarize(numsets = length(SETNO))
...
total <- strat %>% 
  ...
  summarise(biomass = sum(biomass)*0.001,
            mature = sum(mature_abun)*0.001,
            immature = sum(immature_abun)*0.001) %>% 
  pivot_longer(cols = c(mature, immature),
               names_to = "maturity",
               values_to = "abundance")
...
ggplot(total, aes(x=YEAR, y=abundance, col=maturity, shape=maturity))+
  ...
  geom_line()+
  theme_classic()+
  labs(x = "Year", y = "Mean number per tow (thousands)")+
  ggtitle("B")
```

  
```{r, fig.cap="Stratified total biomass (A) and abundance (B) of immature and immature Atlantic wolffish in the Georges Bank RV Survey, 1987-2021.", fig.align='center'}
strat <- georges %>% 
  group_by(YEAR, STRAT) %>% 
  summarize(numsets = length(SETNO))
...
gridExtra::grid.arrange(G,H,I, P, E, D, spring_immature, catches, springVsW_totalcatch, nrow=3, ncol = 3)
```
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Halibut Longline Survey, 1998-2021, summed by 10 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Sentinel Survey, 1996-2020, summed by 10 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Lobster/ITQ Survey, 1995-2021, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Snowcrab Ecosystem Survey, 2004-2021, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Distribution of Atlantic wolffish catches in the Snowcrab Survey, 2004-2021, summed by 5 minute aggregation.", fig.align='center'}
...
```{r, fig.cap="Catch per Unit Effort of Atlantic wolffish in the Sentinel Longline Survey", fig.align='center'}
pue <- sentinel %>% 
  select(YEAR, EST_COMBINED_WT, NUM_HOOK_HAUL) %>% 
  mutate(sum = (EST_COMBINED_WT/NUM_HOOK_HAUL)*1000) %>% 
  group_by(YEAR) %>%   
...
```{r, fig.cap="Catch per Trap of Atlantic wolffish in the ITQ/Lobster Survey", fig.align='center'}
